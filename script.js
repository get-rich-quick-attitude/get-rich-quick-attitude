$(document).ready(function () {
    ...
    function tick(t) {
        if (count < remainingTime) {
            ...
            // Perform these actions in the event that the timer reaches zero
            count = 0;
            empty();
            $('#reset, #clock').hide();
            ...
            if (timeToRest) {
                // Update the on-screen text to indicate that a countdown has completed. Switch the remaining time to the upcoming countdown
...
function fn_sendFB(sns) {
    ...
    var snsTitle = 'Timato/타이머 공유';
    if (sns == 'facebook') {
        var url =
            'http://www.facebook.com/sharer/sharer.php?u=' +
            encodeURIComponent(thisUrl);
        window.open(url, '', 'width=720, height=720');
    } else if (sns == 'twitter') {
        var url =
        ...
        window.open(url, 'tweetPop', 'width=720, height=720,scrollbars=yes');
    } else if (sns == 'band') {
        var url =
            ...
            '&route=' +
            encodeURIComponent(thisUrl);
        window.open(url, 'shareBand', 'width=720, height=720, resizable=yes');
    } else if (sns == 'kakaotalk') {
        Kakao.init('09e8459813c0c5a9d1f7ce9d65847f2b');
        ...
        Kakao.Link.createDefaultButton({
            ...
            objectType: 'feed',
            content: {
                title: 'Timato/Timer', // 보여질 제목
                description: 'Timato/Timer', // 보여질 설명
                imageUrl: 'https://www.timato.co.kr/', // 콘텐츠 URL
                link: {
                    mobileWebUrl: 'https://www.timato.co.kr/',
                    webUrl: 'https://www.timato.co.kr/',
                },
            },
        });
    }
